...
// src/product-search-actor/index.ts
import { Actor } from "./runtime.js";
var ProductSearchActor = class extends Actor {
  constructor(state, env) {
    super(state, env);
  }
  async initialize() {
    const existing = await this.state.storage.get("state");
    if (!existing) {
      await this.state.storage.put("state", { lastSearch: "", timestamp: (/* @__PURE__ */ new Date()).toISOString() });
    }
  }
...
var product_search_actor_default = ProductSearchActor;

// <stdin>
var stdin_default = product_search_actor_default;
export {
  ProductSearchActor,
  cors,
